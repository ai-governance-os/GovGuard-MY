...
- pytest after **product/state/data cleanup batch (CODEX governance brief, F1–F6)** — (F1/F8) post-decision GREEN state copy: after Approve/Reject the summary, workflow panel, per-step row, WORKFLOW chip and 105/107/110 pipeline rows now read "verified (simulated)" / "rejected · database unchanged" instead of staying on "awaiting verification" — driven by a `greenGateState(d)` helper off `step.status` (success=approved else rejected) + `task.status`; the GREEN step's curated notice body is no longer surfaced as the chat answer for a detected workflow. (F2) hid the internal 102W resolver `confidence` from the panel + softened deadline to "review recommended within 24h". (F3) renamed "mother database" → **protected student-record database** everywhere (config/curated/app.js/docs/tests). (F4) athletics-relevant safe alternative in 101D (dropped "homework completion"). (F5) fixed the Xiao Le DB inconsistency — was Silver+PB, now no-medal/below-PB across student_parent_database.md + content_pack.md + the build spec (Ali stays Silver). (F6) FB disclosure is now one short English line (curated + 102b fallback). (Issue #7 SOP-approve was already fixed in `f065ea3`.) +1 data-contract assertion (rejected GREEN step != success). **1009 passed, 1 skipped, 0 failed** (1010 collected; = 949 + 60).
- pytest after **learning/audit polish batch (mentor retest + CODEX learning-audit brief, G1–G4)** — (R1/G1) finished `content_pack.md`: the trilingual FB-post examples still showed Xiao Le with a Silver medal → now "represented in the 100m final" (中文/BM/EN); (R2/G2) the 103 Decision pipeline note is now gate-aware (pending/approved/rejected) instead of always "awaits human verification"; (F5/F6/G3) **differentiated learning cards** — INFEASIBLE reward → "📋 Decision framework produced · no amount guessed", BLUE schedule edit → "🗂 Output version recorded · no long-term memory update", RED-learning → "Personal-data learning blocked" (honest: a framework card + a version artifact really exist) — directly answers "every route looks like it can't learn"; (F1) reworded the "rich; backend" DB note (misreads as wealth) → "Synthetic student-parent records (backend only)…"; (F2) follow-up timing fixed (parents 24h / internal next-day / FB within 3 days after approval) + dropped the generic panel deadline; (R3) the free-text RED-status safe-alternative now matches the athletics-specific 101D one; (F4) stronger release copy ("no real parent message delivered, no Facebook post published"); (F3) centralised-training time 8–11am → 8:00 a.m.–12:30 p.m. (button + planner + test); audit headings reframed as expert tooling (Subject confidence→Governance Classifier Trace, etc.). **1009 passed, 1 skipped, 0 failed** (1010 collected; = 949 + 60).
- pytest after **expert-retest final polish + flaky-test fix (submission audit)** — (1) medal-count PROSE "one gold, **two** silver" → "one gold, **one** silver, a national-level 100m representation, …" (content_pack intro+conclusion + DB description; Xiao Le has no medal); (2) centralised-training time → **8:00 a.m. to 5:00 p.m. daily, with breaks and a supervised lunch** (owner wanted a realistic full-day camp). **A fresh-venv reproduce of the GitHub submission artifact (clean clone → `pip install -e .[dev]` → pytest) caught a FLAKY test** the dev venv masked: `test_skill_lifecycle.py::test_sweep_proposes_supersede_for_same_shape` failed intermittently on a clean checkout (would also fail on CI / a judge's clone). Root cause = a real DETERMINISM bug: `SkillManager._detect_supersede_candidates` ranked same-(category,shape) skills by `created_at` alone; two skills created in the same Windows coarse-clock tick get an IDENTICAL `created_at`, so the "newest wins" tie resolved by stable-sort/iteration order → the WRONG skill could be superseded. FIX: deterministic tie-break by first-append position in the index log (creation order = later wins); proven on an exact tie; +1 regression test. **1010 passed, 1 skipped, 0 failed** (1011 collected; = 949 + 61). LESSON: always do a fresh-venv reproduce of the SUBMISSION artifact before declaring "可提交" — the dev venv + accumulated timing masked a CI-red flaky test.
